document.addEventListener('DOMContentLoaded', () => {
    ...
    async function invokeAction(serviceId, actionName, args = {}) {
        try {
            const response = await fetch(`/api/devices/${udn}/action`, {
                method: 'POST',
    ...
    function showError(message) {
        ...
        [prevBtn, playPauseBtn, stopBtn, nextBtn, muteBtn, volumeSliderEl, progressBarEl].forEach(el => el.disabled = true);
    }
    ...
    function timeToSeconds(timeStr) {
        if (!timeStr || timeStr === 'NOT_IMPLEMENTED' || !timeStr.includes(':')) return 0;
        const parts = timeStr.split(':').map(p => parseInt(p, 10));
         if (parts.length === 3) {
    ...
    function setupEventListeners() {
        playPauseBtn.addEventListener('click', () => {
            ...
            invokeAction('urn:upnp-org:serviceId:AVTransport', action, args).then(pollOnce);
        });

        stopBtn.addEventListener('click', () => {
            invokeAction('urn:upnp-org:serviceId:AVTransport', 'Stop', { InstanceID: '0' }).then(pollOnce);
        });

        nextBtn.addEventListener('click', () => {
            invokeAction('urn:upnp-org:serviceId:AVTransport', 'Next', { InstanceID: '0' }).then(pollOnce);
        });

        prevBtn.addEventListener('click', () => {
            invokeAction('urn:upnp-org:serviceId:AVTransport', 'Previous', { InstanceID: '0' }).then(pollOnce);
        });

        volumeSliderEl.addEventListener('input', () => {
            const volume = volumeSliderEl.value;
            invokeAction('urn:upnp-org:serviceId:RenderingControl', 'SetVolume', { InstanceID: '0', Channel: 'Master', DesiredVolume: volume });
        });

        muteBtn.addEventListener('click', async () => {
            const response = await invokeAction('urn:upnp-org:serviceId:RenderingControl', 'GetMute', { InstanceID: '0', Channel: 'Master' });
            if (response && response.data) {
                const isMuted = response.data.CurrentMute === '1';
                await invokeAction('urn:upnp-org:serviceId:RenderingControl', 'SetMute', { InstanceID: '0', Channel: 'Master', DesiredMute: !isMuted });
        ...
        progressBarEl.addEventListener('input', () => {
            ...
            const targetTime = `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
            invokeAction('urn:upnp-org:serviceId:AVTransport', 'Seek', { InstanceID: '0', Unit: 'REL_TIME', Target: targetTime });
        });
    }
    ...
    async function pollStatus() {
        // Get transport state
        const transportInfo = await invokeAction('urn:upnp-org:serviceId:AVTransport', 'GetTransportInfo', { InstanceID: '0' });
        if (transportInfo && transportInfo.data) {
            updateTransportState(transportInfo.data.CurrentTransportState);
        }

        // Get position info
        const positionInfo = await invokeAction('urn:upnp-org:serviceId:AVTransport', 'GetPositionInfo', { InstanceID: '0' });
        if (positionInfo && positionInfo.data) {
            updatePositionInfo(positionInfo.data);
        }

        // Get volume and mute state
        const volumeInfo = await invokeAction('urn:upnp-org:serviceId:RenderingControl', 'GetVolume', { InstanceID: '0', Channel: 'Master' });
        if (volumeInfo && volumeInfo.data) {
            volumeSliderEl.value = volumeInfo.data.CurrentVolume;
        }
        const muteInfo = await invokeAction('urn:upnp-org:serviceId:RenderingControl', 'GetMute', { InstanceID: '0', Channel: 'Master' });
        if (muteInfo && muteInfo.data) {
            muteBtn.textContent = muteInfo.data.CurrentMute === '1' ? '🔊' : '🔇';
        }
    }
