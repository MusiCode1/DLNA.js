...
exports.playFolderOnRenderer = playFolderOnRenderer;
exports.createRendererHandler = createRendererHandler;
// server/rendererHandler.ts
const express_1 = require("express");
const dlna_core_1 = require("@dlna-tv-play/dlna-core");
const logger = (0, dlna_core_1.createModuleLogger)('rendererHandler');
// הפונקציה הזו תכיל את הלוגיקה המרכזית לניגון תיקייה
// היא תקבל את כל התלויות הנדרשות ותחזיר Promise
async function playFolderOnRenderer(rendererUdn, mediaServerUdn, folderObjectId, activeDevices, parentLogger // הוספת הפרמטר parentLogger
) {
    parentLogger.info(`Attempting to play folder ${folderObjectId} from server ${mediaServerUdn} on renderer ${rendererUdn}`);
    const renderer = getValidatedDevice(rendererUdn, 'Renderer', activeDevices, { status: (code) => ({ send: (body) => { throw { statusCode: code, body }; } }) });
    if (!renderer) {
    ...
    const mediaServer = getValidatedDevice(mediaServerUdn, 'Media Server', activeDevices, { status: (code) => ({ send: (body) => { throw { statusCode: code, body }; } }) });
    if (!mediaServer) {
        const message = `Media Server with UDN ${mediaServerUdn} not found or invalid.`;
        parentLogger.warn(message); // שימוש ב-parentLogger
        return { success: false, message, statusCode: 404 };
    }
    const cdServiceDescriptionOriginal = getValidatedService(mediaServer, 'ContentDirectory', 'ContentDirectory', { status: (code) => ({ send: (body) => { throw { statusCode: code, body }; } }) });
    ...
    const avTransportService = getValidatedService(renderer, 'AVTransport', 'AVTransport', { status: (code) => ({ send: (body) => { throw { statusCode: code, body }; } }) });
    ...
    try {
        ...
        const cdServiceForBrowse = {
            ...cdServiceDescriptionOriginal,
            controlURL: absoluteCdControlURL
        };
        const cds = new dlna_core_1.ContentDirectoryService(cdServiceForBrowse);
        const browseResult = await cds.browse(folderObjectId, dlna_core_1.BrowseFlag.BrowseDirectChildren, '*', 0, 0);
        ...
        const videoItems = browseResult.items.filter((item) => {
        ...
        let firstItemDidlXml = "";
        const getItemUriAndDidl = (itemData, parentIdForDidl) => {
            let currentItemResUrl = null;
            ...
            if (itemData.resources && itemData.resources[0] && itemData.resources[0].uri) {
                ...
                if (itemData.resources[0].protocolInfo)
                    protocolInfo = itemData.resources[0].protocolInfo;
            }
            else if (itemData.res) {
                if (typeof itemData.res === 'string')
                    currentItemResUrl = itemData.res;
                else if (Array.isArray(itemData.res) && itemData.res[0]) {
                    if (typeof itemData.res[0] === 'string')
                        currentItemResUrl = itemData.res[0];
                    else if (itemData.res[0]._ && typeof itemData.res[0]._ === 'string') {
                        currentItemResUrl = itemData.res[0]._;
                        if (itemData.res[0].$ && itemData.res[0].$.protocolInfo)
                            protocolInfo = itemData.res[0].$.protocolInfo;
                    }
                }
            }
...
function getValidatedDevice(udn, deviceType, activeDevices, res) {
...
function getValidatedService(device, serviceTypeIdentifier, serviceFriendlyName, res) {
    const service = device.serviceList.find(// Added non-null assertion
    (s) => s.serviceType?.includes(`urn:schemas-upnp-org:service:${serviceTypeIdentifier}`) || s.serviceId?.includes(`urn:upnp-org:serviceId:${serviceTypeIdentifier}`));
...
async function stopPlayback(avTransportService, rendererUdn) {
...
async function setAvTransportUri(avTransportService, rendererUdn, itemUrl, itemMetadataXml) {
    if (!avTransportService.actionList || !Array.isArray(avTransportService.actionList)) {
...
async function startPlayback(avTransportService, rendererUdn) {
    if (!avTransportService.actionList || !Array.isArray(avTransportService.actionList)) {
...
async function setNextAvTransportUri(avTransportService, rendererUdn, itemUrl, itemMetadataXml) {
    if (!avTransportService.actionList || !Array.isArray(avTransportService.actionList)) {
...
async function executePlaybackCommands(renderer, avTransportService, itemUrl, itemMetadataXml, res, next, itemNameForLog) {
    if (!renderer.baseURL) {
    ...
    try {
        ...
        if (error.soapFault && error.soapFault.upnpErrorCode) {
            ...
            error.customMessage = `SOAP Fault: ${error.soapFault.faultString} (UPnP Code: ${error.soapFault.upnpErrorCode})`;
        }
...
function createRendererHandler(activeDevices) {
    const router = (0, express_1.Router)();
    router.post('/:rendererUdn/play', async (req, res, next) => {
        ...
        try {
            ...
            const cds = new dlna_core_1.ContentDirectoryService(cdServiceForBrowse);
            const browseResult = await cds.browse(objectID, dlna_core_1.BrowseFlag.BrowseMetadata, '*', 0, 1);
            if (!browseResult || browseResult.numberReturned === 0 || !browseResult.items || browseResult.items.length === 0) {
            ...
            const itemData = browseResult.items[0];
            if (itemData.resources && itemData.resources[0] && itemData.resources[0].uri) {
  ...
  <item id="${xmlEscape(objectID)}" parentID="${xmlEscape(itemData.parentId || '-1')}" restricted="1">
    ...
    router.post('/:rendererUdn/play-folder', // נתיב הוחזר למקורי
    async (req, res, next) => {
        const { rendererUdn } = req.params;
        ...
        const avTransportService = getValidatedService(renderer, 'AVTransport', 'AVTransport', res);
        if (!avTransportService)
            return; // זה כבר אמור להיות מטופל על ידי getValidatedService אבל נשאר לבטיחות
        try {
