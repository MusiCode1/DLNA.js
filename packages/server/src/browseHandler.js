...
exports.handleBrowseRequest = void 0;
const dlna_core_1 = require("@dlna-tv-play/dlna-core");
const logger = (0, dlna_core_1.createModuleLogger)('BrowseHandler');
const handleBrowseRequest = async (req, res, next, activeDevices) => {
    const { udn } = req.params;
    const { ObjectID = '0', // ברירת מחדל לשורש
    BrowseFlag: browseFlagParam = 'BrowseDirectChildren', // שינוי שם המשתנה כדי לא להתנגש עם ה-enum
    Filter = '*', StartingIndex = 0, RequestedCount = 0, // 0 בדרך כלל אומר "הכל"
    SortCriteria = '' } = req.body;
    // ולידציה עבור BrowseFlag
    if (!Object.values(dlna_core_1.BrowseFlag).includes(browseFlagParam)) {
        logger.warn(`Invalid BrowseFlag value received: ${browseFlagParam}. Valid values are: ${Object.values(dlna_core_1.BrowseFlag).join(', ')}`);
        res.status(400).json({
            error: 'Invalid BrowseFlag parameter',
            message: `Valid values for BrowseFlag are: ${Object.values(dlna_core_1.BrowseFlag).join(', ')}.`
        });
        return; // Exit after sending response
    }
    const validatedBrowseFlag = browseFlagParam; // עכשיו אפשר להשתמש בזה בבטחה
    ...
    const cdServiceDescriptionOriginal = device.serviceList.find((service) => service.serviceType?.startsWith('urn:schemas-upnp-org:service:ContentDirectory:'));
    ...
    try {
        ...
        const serviceDescriptionForCDS = {
        ...
        const cdService = new dlna_core_1.ContentDirectoryService(serviceDescriptionForCDS);
        logger.info(`Attempting to browse ContentDirectory on ${device.friendlyName} (UDN: ${udn}) with ObjectID: ${ObjectID}`);
        const result = await cdService.browse(ObjectID, validatedBrowseFlag, // שימוש בערך המאומת
        Filter, Number(StartingIndex), Number(RequestedCount), SortCriteria);
                ...
                if (resourceUrl) { // At this point, resourceUrl is a non-empty string
                    ...
                    if (!resourceUrl.startsWith('http://') && !resourceUrl.startsWith('https://')) {
                        try {
                            ...
                            logger.warn(`Failed to construct absolute URL for resource: "${resourceUrl}" with base: "${device.baseURL}". Error: ${e.message}`);
...
exports.handleBrowseRequest = handleBrowseRequest;
