...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const useProxyCb = document.getElementById('use-proxy-cb');
    ...
    // טיפול בתיבת סימון של הפרוקסי
    useProxyCb.addEventListener('change', () => {
        proxyUrlInput.style.display = useProxyCb.checked ? 'block' : 'none';
        localStorage.setItem('use-proxy', useProxyCb.checked.toString());
    });
    
    // שמירת כתובת הפרוקסי
    proxyUrlInput.addEventListener('input', () => {
        localStorage.setItem('proxy-url', proxyUrlInput.value);
    });
    ...
    connectBtn.addEventListener('click', () => {
        ...
        // קביעת פרמטרי הפרוקסי
        const useProxy = useProxyCb.checked;
        const proxyUrl = proxyUrlInput.value.trim() || 'ws://localhost:3005/ws';
        
        remote.connect(ip, key, useProxy, proxyUrl);
    });
    ...
    // טעינת הגדרות הפרוקסי
    const savedUseProxy = localStorage.getItem('use-proxy');
    if (savedUseProxy === 'true') {
        useProxyCb.checked = true;
        proxyUrlInput.style.display = 'block';
    }
    const savedProxyUrl = localStorage.getItem('proxy-url');
    if (savedProxyUrl) {
