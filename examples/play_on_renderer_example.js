...
const express_1 = __importDefault(require("express")); // הוספת ייבוא ל-Express
const node_url_1 = require("node:url");
const wake_and_check_device_1 = require("../drafts/wake_and_check_device");
const src_1 = require("../src");
const didlLiteUtils_1 = require("../src/didlLiteUtils"); // ייבוא הפונקציה החדשה
const logger = (0, src_1.createModuleLogger)('PlayOnRendererExample');
const settingsList = {
    myHome: {
        mediaServer: {
...
const RENDERER_IP_ADDRESS = (new node_url_1.URL(settings.mediaRender.docUrl)).hostname; // נגזר מה-URL של הרינדור
 ...
 */
function findService(device, serviceTypeOrId) {
    if (!device.serviceList) {
        logger.warn(`Device ${device.friendlyName} has no service list.`);
        return undefined;
    }
    return device.serviceList.find((s) => s.serviceType === serviceTypeOrId || s.serviceId === serviceTypeOrId);
}
/**
 ...
 */
function findAction(service, actionName) {
...
async function main() {
    ...
    try {
        await (0, wake_and_check_device_1.wakeDeviceAndVerify)(RENDERER_MAC_ADDRESS, RENDERER_IP_ADDRESS, WAKE_ON_LAN_ADDRESS, // broadcast address
        9, // wol port
    ...
    let mediaServerDevice = null;
    try {
        const processedServer = await (0, src_1.processUpnpDeviceFromUrl)(MEDIA_SERVER_URL, src_1.DiscoveryDetailLevel.Full);
        if (processedServer && processedServer.detailLevelAchieved === src_1.DiscoveryDetailLevel.Full) {
            mediaServerDevice = processedServer;
    ...
    let rendererDevice = null;
    try {
        const processedRenderer = await (0, src_1.processUpnpDeviceFromUrl)(RENDERER_DEVICE_XML_URL, src_1.DiscoveryDetailLevel.Full);
        if (processedRenderer && processedRenderer.detailLevelAchieved === src_1.DiscoveryDetailLevel.Full) {
            rendererDevice = processedRenderer;
    ...
    try {
        ...
        const cds = new src_1.ContentDirectoryService(cdsServiceInfo);
        logger.debug(`Attempting to browse directory ID "${TARGET_BROWSE_DIRECTORY_ID}" using ContentDirectoryService.`);
        const browseResult = await cds.browse(TARGET_BROWSE_DIRECTORY_ID, src_1.BrowseFlag.BrowseDirectChildren, '*', // Filter
        0, // StartingIndex
        ...
        if (browseResult && browseResult.items) {
            mediaItems = browseResult.items.filter((item) => item.class.startsWith('object.item.audioItem') || item.class.startsWith('object.item.videoItem'));
        }
        ...
        try {
            if (i === 0) {
                ...
                const setUriArgs = {
                    InstanceID: 0,
                    CurrentURI: mediaUrl,
                    CurrentURIMetaData: (0, didlLiteUtils_1.createSingleItemDidlLiteXml)(item, resource),
                };
                ...
                const setNextUriArgs = {
                    InstanceID: 0,
                    NextURI: mediaUrl,
                    NextURIMetaData: (0, didlLiteUtils_1.createSingleItemDidlLiteXml)(item, resource),
                };
